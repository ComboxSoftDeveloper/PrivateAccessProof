...
dotnet run -c Release -f net10.0 --no-build -- --filter *ReadBench* noasm > "%OUT%\probe_noasm.txt" 2>&1
set PROBE1=%ERRORLEVEL%
if exist "Bdn\PrivateAccessProof.log" copy /Y "Bdn\PrivateAccessProof.log" "%OUT%\probe_noasm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
if not "%PROBE1%"=="0" echo   завершилось с ошибкой, смотри "%OUT%\probe_noasm.txt"

echo.
echo === Проба: один класс с дизассемблером ===
dotnet run -c Release -f net10.0 --no-build -- --filter *ReadBench* > "%OUT%\probe_asm.txt" 2>&1
set PROBE2=%ERRORLEVEL%
if exist "Bdn\PrivateAccessProof.log" copy /Y "Bdn\PrivateAccessProof.log" "%OUT%\probe_asm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
...
if exist "Bdn\PrivateAccessProof.log" copy /Y "Bdn\PrivateAccessProof.log" "%OUT%\bench.log" >nul
